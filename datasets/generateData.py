...
if __name__ == '__main__':
    # 导入原始数据文件，该文件中有8个图像切片，每一个的尺寸是512*512，每一个点的取值为0-1的数
    image_path = 'CT_images_preprocessed.mat'
    num_dete = 512
    # 先只用验证集来验证模型在已有数据上的性能
    num_val_proj = 60
    # 设置投影的角度
    for num_proj in [60]:
            # 选择需要进行投影的图像索引
            for img_idx in [5]:
                ...
                # 计算幅值和相位
                magnitude = np.abs(recon_img)
                phase = np.angle(recon_img)

                # 可视化
                fig, ax = plt.subplots(figsize=(5.12, 5.12))
                ax.imshow(phase, cmap='gray')
                # ax.imshow(inverse_result.imag, cmap='gray')
                print("max(np.abs(inverse_result))", np.max(np.abs(img)))
                # 关闭坐标轴
                ax.axis('off')
                # 调整子图布局，去掉所有边距
                ...
                # fig = plt.figure(frameon=False)
                # fig, ax = plt.subplots(figsize=(5.12, 5.12), dpi=100)  # 去掉图像周围的边框
                # plt.imshow(img.imag)  # 灰度图显示
                # plt.axis('off')  # 去掉坐标轴
                # plt.subplots_adjust(left=0, right=1, top=1, bottom=0)  # 去掉外部的白边
                # # 显示或保存结果
                # plt.show()

                # img_real = y_func(shift_img.real, 2 / np.max(shift_img.real))
                # print("np.max(shift_img.real)", np.max(shift_img.real))
                # print("np.min(shift_img.real)", np.min(shift_img.real))
                # img_imag = y_func(shift_img.imag, 2 / np.max(shift_img.imag))
                # print("np.max(shift_img.imag)", np.max(shift_img.imag))
                # print("np.min(shift_img.imag)", np.min(shift_img.imag))
                # img_fre = img_real + 1j * img_imag
                # print("np.max(img_fre.real)", np.max(img_fre.real))
                # print("np.min(img_fre.real)", np.min(img_fre.real))
                # print("np.max(img_fre.imag)", np.max(img_fre.imag))
                # print("np.min(img_fre.imag)", np.min(img_fre.imag))
                # print("\n")
                #

                # # ========对图像逆变换操作的验证====================
                # # 暂时只能根据生成数据集时的超参数来写
                # measurements_real = x_func(img_fre.real, np.max(shift_img.real)/ 2)
                # print("np.max(img_fre.real)", np.max(img_fre.real))
                # print("np.min(img_fre.real)", np.min(img_fre.real))
                # measurements_imag = x_func(img_fre.imag, np.max(shift_img.imag) / 2)
                # print("np.max(img_fre.imag)", np.max(img_fre.imag))
                # print("np.min(img_fre.imag)", np.min(img_fre.imag))
                # measurements = measurements_real + 1j * measurements_imag
                # print("np.max(measurements.real)", np.max(measurements.real))
                # print("np.min(measurements.real)", np.min(measurements.real))
                # print("np.max(measurements.imag)", np.max(measurements.imag))
                # print("np.min(measurements.imag)", np.min(measurements.imag))
                # print("\n")
                # # ============对原图像的逆归一化验证=================
                ...
                proj_sino = projs
                # 图像范围是0-1
                proj_sino = np.fft.fftshift(proj_sino)
                f_ps = np.fft.fft(proj_sino)
                f_ps = np.fft.ifftshift(f_ps)
                # # 对 f_ps 的每一行进行一维傅里叶变换的移位操作
                # # 创建一个与 f_ps 形状相同的数组来存储移位后的结果
                # f_ps = np.zeros_like(f_p, dtype=complex)
                #
                # # 使用 for 循环对每一行进行移位操作
                # for i in range(f_p.shape[0]):
                #     f_ps[i] = np.fft.fftshift(f_p[i])
                ...
                # ============================================模拟插值=======================================
                dstxy = np.column_stack((dstx, dsty))
                points = (src_point, dstxy)
                # m = (dstx, dsty)
                f_p = f_ps.flatten()
                # p = points[0]
                # q = points[1]
                ...
                # phase = np.angle(Fourier2_np)
                # # 仅保留相位信息，幅值置为 1
                # phase_only = 10 * np.exp(1j * phase)
                # # 进行逆傅里叶变换
                # phase_only_img = np.fft.ifft2(np.fft.ifftshift(phase_only))
                # phase_only_img = np.fft.ifftshift(phase_only_img)
                # phase_only_img = np.abs(phase_only_img)
                # print("相位逆变换图像原始数值范围:", np.min(phase_only_img), np.max(phase_only_img))
                # # 归一化相位图像
                # phase_only_img = (phase_only_img - np.min(phase_only_img)) / (
                ...
                fig, ax = plt.subplots(figsize=(5.12, 5.12))
                plt.imshow(np.abs(inverse_result), cmap='gray')
                plt.axis('off')
                ...
                proj_tensor = np.fft.fftshift(proj_sino)
